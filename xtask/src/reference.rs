...
const BANNER: &str = "<!-- generated by `cargo xtask docs` from docs/contracts/. Do not edit by hand: \
...
/// Generates every reference page from the registries under `root/docs/contracts`.
///
/// # Errors
///
/// Returns a [`GenerateError`] if `docs/contracts/` is missing or a registry is not valid YAML.
...
fn index_page() -> String {
    let mut page = header(
        ...
        "Derived from the machine-readable contracts in `docs/contracts/`, which are the public \
    ...
    page.push_str(
        "\n| Page | Contract |\n\
         |---|---|\n\
         | [Commands](commands.md) | `docs/contracts/commands/*.yaml` — every command, its schemas, \
         selectors, options and examples |\n\
         | [Verbs](verbs.md) | `docs/contracts/verbs.yaml` — the controlled vocabulary of spec §7 |\n\
         | [Targets](targets.md) | `docs/contracts/targets.yaml` — the nouns of spec §8 |\n\
         | [Schemas](schemas.md) | `docs/contracts/schemas/*.v1.yaml` — the object contracts of spec §28 |\n\
         | [Adapters](adapters/README.md) | `docs/contracts/adapters/first-party/*.yaml` — what each \
         external tool adapts, at which versions, through which invocations (spec v0.3 §1.66) |\n\
         | [Streaming](streaming.md) | `docs/contracts/hardening/streaming_classification.yaml` — what \
         each pipeline operation may do to a stream, and what the order of events means (v0.4.1 \
         Appendix E, §27) |\n\
         | [Terminology](terminology.md) | `docs/contracts/hardening/terminology.yaml` — the eight \
         security terms of v0.4.1 §19.1 and what each one promises |\n\
         | [Remote trust](remote-trust.md) | `docs/contracts/hardening/remote_trust.yaml` — the six \
         things a remote link keeps apart, and what each one does not establish (v0.4.1 §51.3) |\n\
         | [Security boundaries](security-boundaries.md) | \
         `docs/contracts/hardening/security_boundaries.yaml` — where untrusted input enters, what has \
         to hold before it goes on, which crate owns it and the test that proves the refusal \
         (v0.4.1 §6.1, §6.2, §20) |\n\
         | [Release verification](release-verification.md) | \
         `docs/contracts/hardening/release_verification.yaml` — how to check a release's checksums, \
         signature and provenance before installing it (v0.4.1 §47.5, §67.7) |\n\
         | [Errors](errors.md) | `docs/contracts/errors.yaml` — the stable taxonomy of spec §43 |\n\
         | [Capabilities](capabilities.md) | `docs/contracts/capabilities.yaml` — provider and KUANG/11 \
         capabilities |\n\
...
fn streaming_page(document: Option<&Yaml>) -> String {
    let mut page = header(
        ...
        "What each pipeline operation may do to a stream, and what a consumer may conclude from \
         ...
         `docs/contracts/hardening/streaming_classification.yaml`, which is also what `explain` reads \
...
fn terminology_page() -> String {
    let mut page = header(
        ...
        "Eight words with fixed meanings (v0.4.1 \u{a7}19.1). They mean this in the README, in the \
         ...
         `docs/contracts/hardening/terminology.yaml`, this page is rendered from it, and \
...
fn remote_trust_page() -> String {
    let mut page = header(
        ...
        "Six things a remote link keeps apart (v0.4.1 \u{a7}51.3). Each says what a reader may \
         ...
         `docs/contracts/hardening/remote_trust.yaml`, which the documentation gate reads as well.",
    );
...
fn security_boundaries_page(document: Option<&Yaml>) -> String {
    let mut page = header(
        ...
        "Where untrusted input enters Ono-Sendai, what must hold before it goes further, the one \
         ...
         `docs/contracts/hardening/security_boundaries.yaml`, which the contract gate reads as well: \
...
fn release_verification_page() -> String {
    let mut page = header(
        ...
        "Every release publishes the digest of every artifact, a signature over that manifest and \
         ...
         anything. Rendered from `docs/contracts/hardening/release_verification.yaml`, which the README \
...
fn adapters_matrix_page(packs: &[ono_adapter::AdapterPack]) -> String {
    let mut page = header(
        ...
        "The compatibility matrix of spec v0.3 §1.66, derived from `docs/contracts/adapters/first-party/*.yaml`. \
            ...
            Invocation::Script(script) => {
                ...
                for capability in granted_capabilities(script) {
                    if registry.capability(&capability).is_none() {
                        problems.push(problem(format!(
                            "grants `{capability}`, which `docs/contracts/capabilities.yaml` does not \
