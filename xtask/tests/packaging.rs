...
fn should_publish_the_same_bytes_package_validation_installed() {
    ...
    for (name, bytes) in [
        ("ono_0.4.3_amd64.deb", "the amd64 package"),
        ("ono-0.4.3-1.x86_64.rpm", "the x86_64 rpm"),
    ] {
        std::fs::write(release.join(name), bytes).expect("an artifact");
    }
    let record = |directory: &Path| -> String {
        ["ono_0.4.3_amd64.deb", "ono-0.4.3-1.x86_64.rpm"]
            .iter()
            .map(|name| {
    ...
    std::fs::write(
        release.join("ono_0.4.3_amd64.deb"),
    ...
    assert!(
        !verified && report.contains("ono_0.4.3_arm64.deb"),
