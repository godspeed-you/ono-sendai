...
# committed. Runs identically on a developer machine, in CI and inside the container; CI runs its
# steps in parts, each on a runner of its own, and the parts together are this script (ADR-0852).
#
...
if runs tests; then
  ...
  # explicit skip-verification step." The expectation is declared for the canonical CI
  # environment, so it is enforced there and reported everywhere else — a developer machine
  # without systemd legitimately skips what CI does not, and a gate that failed on it would teach
  # people to skip the gate. The check compares the whole register with one run, so a part that
  # saw only some of the tests leaves it to the step that reads every part's log.
  if [[ "$part" != all ]]; then
    printf 'gate: the skip verification reads every part'"'"'s log, so CI runs it once after all of them (ADR-0852)\n'
  elif [[ "${ONO_CANONICAL_CI:-0}" == "1" ]]; then
...
if runs static; then
  ...
  #
  # The per-input ceiling is loose here on purpose. The default of two seconds is for a campaign
  # on a quiet machine; the gate may run on a loaded one, and a red gate that means "the machine
  # was busy" is worse than a slow input that the next campaign catches. What the gate is for is
  # the crashes.
  step "fuzz"
  ...
  # crate come from a source we approved. It runs here rather than only in CI because a
  # dependency is chosen on a developer machine, and an advisory found after the push is an
  # advisory found after the decision (ADR-0449).
  step "supply chain"
