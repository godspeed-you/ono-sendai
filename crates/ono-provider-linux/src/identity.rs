...
impl IdentityProvider {
    ...
    async fn user_change(&self, action: &Action) -> Result<Vec<AccountCommand>, ErrorValue> {
        ...
        // for: the database is readable by anyone, so the provider can look first (ADR-0104).
    ...
    async fn group_change(&self, action: &Action) -> Result<Vec<AccountCommand>, ErrorValue> {
        ...
        // As for a user: a group that is not there is answered before privilege (ADR-0104).
