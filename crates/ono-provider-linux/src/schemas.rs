...
//!
//! Every one of them is a schema of `docs/contracts/schemas/`, which AGENTS.md §5 makes the public
...
/// `ono.process/1` — `docs/contracts/schemas/process.v1.yaml`.
#[must_use]
pub fn process_id() -> SchemaId {
    SchemaId::new("ono.process", 1)
}

/// `ono.process-detail/1` — `docs/contracts/schemas/process-detail.v1.yaml`.
#[must_use]
pub fn process_detail_id() -> SchemaId {
    SchemaId::new("ono.process-detail", 1)
}

/// `ono.file/1` — `docs/contracts/schemas/file.v1.yaml`.
#[must_use]
pub fn file_id() -> SchemaId {
    SchemaId::new("ono.file", 1)
}

/// `ono.user/1` — `docs/contracts/schemas/user.v1.yaml`.
#[must_use]
pub fn user_id() -> SchemaId {
    SchemaId::new("ono.user", 1)
}

/// `ono.group/1` — `docs/contracts/schemas/group.v1.yaml`.
#[must_use]
pub fn group_id() -> SchemaId {
    SchemaId::new("ono.group", 1)
}

/// `ono.mount/1` — `docs/contracts/schemas/mount.v1.yaml`.
#[must_use]
pub fn mount_id() -> SchemaId {
    SchemaId::new("ono.mount", 1)
}

/// `ono.filesystem/1` — `docs/contracts/schemas/filesystem.v1.yaml`.
#[must_use]
pub fn filesystem_id() -> SchemaId {
    SchemaId::new("ono.filesystem", 1)
}

/// `ono.device/1` — `docs/contracts/schemas/device.v1.yaml`.
#[must_use]
pub fn device_id() -> SchemaId {
    SchemaId::new("ono.device", 1)
}

/// `ono.env-var/1` — `docs/contracts/schemas/env-var.v1.yaml`.
#[must_use]
...
/// registry, which means a file under `docs/contracts/schemas/` stopped loading. Every provider entry
...
pub fn require(id: &SchemaId) -> Result<Arc<Schema>, ErrorValue> {
    registry().get(id).ok_or_else(|| {
        ...
        .with_help(
            "`docs/contracts/schemas/` is where the contract lives; `cargo xtask spec-check` \
