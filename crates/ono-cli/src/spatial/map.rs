...
impl CommandImpl for Map {
    ...
    fn invoke_async<'a>(&'a self, ctx: &'a mut Invocation<'_>) -> OutcomeFuture<'a> {
        Box::pin(async move {
            ...
            // Only the explicit flag reaches this far. `spatial.map.live = true` says how a
            // *view* behaves; where there is no view it has nothing to say, and a setting that
            // turned every script's `map` into an endless stream would be a trap rather than a
            // preference (§47, §29.1).
            if asked_live {
