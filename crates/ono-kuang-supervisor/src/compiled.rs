...
/// The operator's own store: `$XDG_CACHE_HOME/ono/kuang/compiled`, or `~/.cache/ono/...`, as
/// this process's environment names it.
///
...
/// The operator's own store as an environment with these two variables names it: the shell
/// passes its session's, which is the environment `kuang-compile` typed in it receives.
#[must_use]
pub fn user_store_in(xdg_cache_home: Option<&OsStr>, home: Option<&OsStr>) -> Option<PathBuf> {
    let absolute =
        |value: Option<&OsStr>| value.map(PathBuf::from).filter(|path| path.is_absolute());
    let cache =
...
pub fn stores() -> Vec<PathBuf> {
    stores_in(user_store())
}

/// The stores searched when the operator's own is `user_store`: it, then the system's.
#[must_use]
pub fn stores_in(user_store: Option<PathBuf>) -> Vec<PathBuf> {
    user_store
        .into_iter()
