...
//!
//! The wait is bounded. A `btrfs` command against a filesystem whose device has stopped
//! answering blocks in the kernel, and an unbounded wait turns that into a shell that never
//! returns a prompt. The runner waits [`ProcessRunner::timeout`], then kills the child and
//! refuses — the same rule `ono-recovery-zfs`'s runner follows (ADR-0805, ADR-0833).
//!
...
/// How long a Btrfs command may run before the runner gives up on it.
///
/// `subvolume show`, `subvolume list` and `filesystem show` read metadata the kernel already
/// holds, and `subvolume snapshot` is copy-on-write; they return in milliseconds. A command still
/// running after thirty seconds is waiting on hardware, and §56.3's direction is to say so.
...
impl ProcessRunner {
    ...
    /// This runner, waiting at most `timeout` for any one program.
    #[must_use]
...
impl ToolRunner for ProcessRunner {
    fn run(&self, program: &str, argv: &[&str]) -> Result<ToolOutput, ErrorValue> {
        ...
        // The streams are drained on threads of their own: a child that fills a pipe buffer blocks
        // in `write`, and a parent that waits for exit before reading would deadlock with it.
        ...
        let status = loop {
            match child.try_wait() {
                        ...
                        return Err(error::tool_failed(
                            ...
                            &format!(
                                "v0.6 §12.3: it did not finish within {:?} and was terminated. A \
                                 Btrfs command that runs this long is waiting on hardware, and \
                ...
                Err(failure) => {
                    return Err(error::tool_failed(
                        program,
                        &format!("waiting for it to finish failed: {failure}"),
                    ));
                }
            }
        };

        // A signalled child has no exit code; §2.14 forbids reading "it stopped" as "it worked".
        Ok(ToolOutput::new(
            status.code().unwrap_or(-1),
            String::from_utf8_lossy(&stdout.recv().unwrap_or_default()).into_owned(),
            String::from_utf8_lossy(&stderr.recv().unwrap_or_default()).into_owned(),
        ))
    }
...
fn drain<R: Read + Send + 'static>(stream: Option<R>) -> mpsc::Receiver<Vec<u8>> {
