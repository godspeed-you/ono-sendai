...
//!   destroy newer snapshots or bookmarks, each destruction is a named action an operator
//!   accepts, and where a clone stands in the way the plan says so and stops.
...
pub use provider::{
    CLONE_MOUNT_ROOT, CP, GUID_FINGERPRINT, MOUNTINFO, PROVIDER_ID, RootDatasetCase,
