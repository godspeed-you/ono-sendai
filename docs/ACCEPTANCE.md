...
- [x] **The twelve configuration settings of §33 are typed, inspectable and default as specified.**
...
- [x] **`at -1m` enters historical context and the prompt says so.** The prompt carries the
...
- [x] **`now` returns to the present and keeps the place when it still exists.** Where the
      ...
      `crates/ono-cli/tests/temporal_context.rs`, case `230-temporal-at-and-now`.
- [x] **An invalid or ambiguous time leaves the context untouched.** `at` resolves before it
...
- [x] **A native mutation in past context answers `temporal.read_only`.** Nothing is changed, and
      the message names `now` as the way back (§4.7) —
      `crates/ono-cli/tests/temporal_read_only.rs`, case `232-temporal-past-is-read-only`.
- [x] **The read-only rule covers KUANG/11 and remote mutations too.** A plugin mutation tool and
...
- [x] **An arbitrary external command in past context answers `temporal.present_only`.** It does
...
- [x] **`present printf ok` executes and preserves the historical coordinate.** The result
      metadata makes the present-bound execution visible at least once (§4.8) —
      `crates/ono-cli/tests/temporal_read_only.rs`, case `233-temporal-present-escape`.
- [x] **A pure Ono transform over reconstructed values still runs in past context.**
...
- [x] **`timeline` at a service place excludes the firehose by default.** The default scope is
...
- [x] **`timeline --all` widens to the visible scope.** Subject to retention and permission —
      `crates/ono-cli/tests/timeline.rs`, case `236-timeline-relevance`.
- [x] **`timeline` is pipeline-compatible.** `timeline --since 1h | where kind ==
      "object.changed" | where subject.type == "service"` yields typed values (§11.4) —
      `crates/ono-cli/tests/timeline.rs`, case `235-timeline-is-typed`.
- [x] **An event reference is stable, inspectable and usable.** `@e42` survives into `inspect
      event`, `at event` and `why event` (§11.6) — `crates/ono-cli/tests/timeline.rs`,
      case `237-event-references`.
- [x] **A recorder downtime interval is rendered as a gap.** The gap is shown even though events
...
- [x] **`find event` answers a predicate without a timestamp being known.** It reuses `find` and
      Ono expression semantics and returns a stream of events (§20.3) —
      `crates/ono-cli/tests/find_event.rs`, case `239-find-event`.
- [x] **Historical context centres the default timeline on the coordinate.** ±15 minutes unless
      retention or configuration narrows it (§11.8) —
      `crates/ono-temporal-query/tests/timeline.rs`.
- [x] **`now()` and `context.time` are different things and both are documented.** `now()` is
...
- [x] **Command history and the temporal ledger stay separate.** Ctrl-R remains command recall;
...
- [x] **A checkpoint plus later change reconstructs the earlier service state.** Nearest trusted
...
- [x] **A PID reused later does not resolve as the historical process.** The historical reference
...
- [x] **A relation added after `T` does not appear in a map at `T`.** Current-only exits do not
      leak into a historical neighbourhood (§14.3) —
      `crates/ono-temporal-reconstruct/tests/relations.rs`, case `242-historical-map`.
- [x] **Absence is reported only where coverage can support it.** A point sample at 12:00 cannot
...
- [x] **Sparse evidence produces unknown, never fake completeness.** A state between two
...
- [x] **A historical collection says whether it is the complete set.** `get process` at `T`
...
- [x] **History outside retention answers `temporal.out_of_retention`.** Distinct from
...
- [x] **A reconstructed object keeps its own schema and gains temporal metadata.** A historical
...
- [x] **Coverage is composed per capability, never as one global label.** `inspect` exposes the
...
- [x] **A place that did not exist at `T` says so and stays where it is.** `look` reports `place
...
- [x] **`changes --since` reports added, removed and changed objects with typed values.**
      Five change classes, each an `ono.temporal-change/1` (§13.2) —
      `crates/ono-cli/tests/changes.rs`, case `245-changes`.
- [x] **A partial before-state is unknown, never zero or empty.** The field reports `from
...
- [x] **v0.4 `look`'s recent-change section is backed by the temporal engine.** One change
      implementation, not two (§13.5) — `crates/ono-cli/tests/spatial_look_changes.rs`,
      case `247-look-changes-are-temporal`.
- [x] **`changes` is pipeline-compatible.** `changes --since 30m | group subject.object_type`
      (§28.1) — `crates/ono-cli/tests/changes.rs`, case `245-changes`.

#### 4.11.6 Causality (§15, §16, §17, §26, §48.5 scenarios 22–27)

- [x] **An Ono `restart service` creates an ActionId and records the systemd job it became.**
...
- [x] **`why` on the resulting transition returns a causal chain.** Every edge names its rule,
      its source and its evidence (§16.5, §15.8) — `crates/ono-temporal-query/tests/why.rs`,
      case `248-action-causality`.
- [x] **A config change shortly before a failure is correlation and stays correlation.** It
...
- [x] **A preceding unrelated event never appears under `known cause`.**
      `crates/ono-temporal-query/tests/correlation.rs`, case `249-correlation-is-not-causation`.
- [x] **Unknown cause is a successful typed explanation.** `cause: null` with evidence,
      correlations and gaps, and no error code (§16.7, §34) —
      `crates/ono-temporal-query/tests/why.rs`, case `250-unknown-cause`.
- [x] **Every causal edge names its rule, source and evidence.** A `CausalLink` cannot be
...
- [x] **`why` works without a model configured.** No AI is required for any core temporal
      feature (§16.1, §55.8) — `crates/ono-cli/tests/why.rs`, case `250-unknown-cause`.
- [x] **`why` refuses ambiguity rather than choosing.** Several equally relevant transitions
      answer `temporal.ambiguous_event` listing the references (§16.3) —
      `crates/ono-temporal-query/tests/why.rs`, case `251-causal-evidence`.
- [x] **The three forms of `why` all work.** `why <target> <selector>`, `why event <ref>` and
      `why field <name>` (§16.2) — `crates/ono-cli/tests/why.rs`, case `248-action-causality`.
- [x] **Temporal proximity alone never produces `caused_by`.** A property test over generated
      event pairs with no shared evidence — `crates/ono-temporal-query/tests/correlation.rs`.
- [x] **The renderer never uses causal wording for a non-causal edge.** `because`, `therefore`,
...
- [x] **An AI hypothesis cannot become a canonical causal edge.** It is an `Inference` with its
...
- [x] **`map --live` pauses without stopping ingestion.** Space freezes the view's cursor; the
      providers, the recorder and the machine keep going (§18.2) —
      `crates/ono-cli/tests/temporal_view.rs`, case `253-rewind-pause`.
- [x] **Stepping backward moves the topology to the previous significant event.** `[` and `]`
...
- [x] **Stepping into a coverage gap shows the gap.** No last state with a silently advancing
      timestamp (§18.6) — `crates/ono-temporal-render/tests/gap_frame.rs`,
      `crates/ono-cli/tests/temporal_view.rs`, case `255-rewind-gap`.
- [x] **Enter on a past map node enters that historical place.** The session's coordinate follows
      the cursor (§18.3) — `crates/ono-cli/tests/temporal_view.rs`, case `254-rewind-stepping`.
- [x] **`N` returns to now and summarises what changed.** The summary uses the canonical
      `changes` engine (§18.7) — `crates/ono-cli/tests/temporal_view.rs`, case `256-return-to-now`.
- [x] **Terminal resize during the temporal view changes no semantic time or place.**
      `crates/ono-cli/tests/temporal_view.rs`, case `256-return-to-now`.
- [x] **The full-screen timeline opens, navigates and exits cleanly.** `timeline --view` and `T`
      from a map; the keys of §19.3; the terminal is restored on every exit path including
      Ctrl-C — `crates/ono-cli/tests/timeline_view.rs`, `crates/ono-cli/tests/temporal_view.rs`,
      case `253-rewind-pause`, case `276-the-full-screen-timeline`. `M` (map at event time) and
      `C` (correlations) are the two keys of §19.3 that are not built; each says so at the view
      rather than pretending, and ADR-0781 records why — `M` needs the terminal hand-off v0.8's
      Deck specifies, and `C` needs a correlation field the timeline record does not carry.
- [x] **Dense event sets are grouped with their hidden counts and time spans preserved.**
...
- [x] **No frame is interpolated between unsupported states.** Semantic state changes only at
...
- [x] **The recorder is disabled by default.** A fresh installation retains nothing and creates
      no store (§10.2) — `crates/ono-cli/tests/recorder.rs`, case `257-recorder-is-opt-in`.
- [x] **`start recorder` creates a private ledger with the specified permissions.** `0700` on the
      directory and `0600` on the database (§30.2) —
      `crates/ono-temporal-ledger/tests/permissions.rs`, case `258-recorder-privacy`.
- [x] **Events survive a shell restart.** Appended before, queryable after (§56.6) —
      `crates/ono-temporal-ledger/tests/persistence.rs`, case `259-recorder-persistence`.
- [x] **A recorder restart marks its downtime as a gap.** Never as continuity (§44.1, §55.5) —
      `crates/ono-recorder/tests/downtime.rs`, case `260-recorder-gap`.
- [x] **Retention expires old data and leaves no dangling reference.** Age and size bounds, and
...
- [x] **A secret never appears in the ledger's bytes.** A byte scan of the database file after a
      command carrying a secret (§30.3, §17.5) — `crates/ono-temporal-ledger/tests/privacy.rs`,
      case `258-recorder-privacy`.
- [x] **Process argv is not persisted by default.** `temporal.record.process_argv` is `false` and
...
- [x] **A corrupted ledger disables the affected history and leaves the shell working.** The
...
- [x] **The store migrates, and every shipped version's fixture is tested.** Migration preserves
...
- [x] **The recorder gains no visibility the user does not already have.** No setuid, no sudo, no
      privileged daemon (§10.5) — `crates/ono-recorder/tests/privilege.rs`,
      case `257-recorder-is-opt-in`.
- [x] **Every ingestion path is bounded and an overflow becomes a gap.** No unbounded channel
...
- [x] **`remove temporal-history` clears the local store under destructive-operation policy.**
      (§30.8) — `crates/ono-cli/tests/recorder.rs`, case `261-retention-and-corruption`.
- [x] **The in-memory session ledger is bounded and its eviction is visible.** 100 000 events by
...
- [x] **Remote source time and local ingest time stay distinct.** The local ledger never
      overwrites one with the other (§24.2) — `crates/ono-remote/tests/temporal.rs`,
      case `262-remote-clocks`.
- [x] **Clock skew alone never creates a causal edge.** Two hosts whose wall clocks differ
      ...
      case `262-remote-clocks`.
- [x] **Explicit cross-host transaction evidence may create a causal chain.** A shared connection
...
- [x] **A remote host with no history says so.** It does not present current state as past state
      (§24.5) — `crates/ono-remote/tests/temporal.rs`, case `264-remote-no-history`.
- [x] **Temporal capability negotiation reports what the peer has.** An absent temporal capability
      degrades the link rather than failing it (§24.1) —
      `crates/ono-protocol/tests/handshake_temporal.rs`, case `264-remote-no-history`.
- [x] **Clock uncertainty is rendered where it matters.** The uncertainty travels with the event
...
- [x] **A remote mutation from historical context is refused at the agent.** The refusal names the
...
- [x] **A plugin without `temporal.read.history` cannot query retained history.** Current object
      read does not imply historical access (§30.7) —
      `crates/ono-kuang-sdk/tests/conformance.rs`, case `265-kuang-temporal-permissions`.
- [x] **A plugin without `temporal.contribute.causality` cannot add a causal link.**
      `crates/ono-kuang-sdk/tests/conformance.rs`, case `265-kuang-temporal-permissions`.
- [x] **A plugin's correlation remains correlation in core rendering.** Plugin causal strength
...
- [x] **A contributed event outside the permitted spatial scope is rejected.** A plugin cannot
...
- [x] **Contributed events are validated for schema, identity, timestamps, size and rate.**
      (§37.3) — `crates/ono-kuang-supervisor/tests/temporal.rs`,
      case `266-kuang-event-contribution`.
- [x] **A contributed historical provider maps into canonical objects with coverage and
      provenance.** (§37.5) — `crates/ono-kuang-testhost/tests/temporal_package.rs`,
      case `267-kuang-history-provider`.
- [x] **A malformed plugin temporal payload is refused without destabilising the host.**
      `fuzz/src/targets.rs` covers the decoder — `fuzz/tests/corpus.rs`,
      case `266-kuang-event-contribution`.

#### 4.11.11 Performance and scale (§32, §49)

- [x] **A deterministic fixture ledger of the declared cardinality exists and builds.** At least
...
- [x] **Startup with temporal persistence disabled is measured and within budget.** Under 5 ms
...
- [x] **Recorder idle overhead is measured.** (§32.4) —
      `crates/ono-recorder/tests/retention_load.rs`, `cargo run -p xtask -- perf`,
      case `270-a-ledger-on-disk-costs-a-disabled-shell-nothing`.
- [x] **The eight measurements of §49 are recorded against the fixture.** Startup, recorder idle
...
- [x] **v0.4's current-state budgets do not regress.** (§32.2) —
...
- [x] **The temporal query budgets of §32.3 hold in the container.** A 15-minute timeline, a
...
- [x] **A long historical query is cancellable and Ctrl-C leaves no lock held.** (§32.6) —
      `crates/ono-cli/tests/temporal_cancellation.rs`,
      `crates/ono-temporal-ledger/tests/cancellation.rs`, case `275-temporal-cancellation`.
- [x] **Retention cleanup runs as bounded background work.** It does not block the prompt
      (§31.8, §31.9) — `crates/ono-recorder/tests/retention_load.rs`, case `272-retention-load`.

#### 4.11.12 The twenty core invariants (§2)

- [x] **Time is explicit.** Ono can always report whether a query was evaluated at `now` or at a
      historical coordinate — `crates/ono-cli/tests/temporal_context.rs`.
- [x] **The past is read-only**, across native, KUANG/11 and remote mutations —
      `crates/ono-cli/tests/temporal_read_only.rs`.
- [x] **History requires evidence.** No historical fact is presented without provenance or a
      documented reconstruction rule — `crates/ono-temporal-reconstruct/tests/provenance.rs`.
- [x] **Absence is not default knowledge** — `crates/ono-temporal-reconstruct/tests/absence.rs`.
- [x] **Coverage is first-class** — `crates/ono-temporal-core/tests/coverage.rs`.
- [x] **Gaps remain gaps.** No interpolation across an unobserved interval —
      `crates/ono-temporal-reconstruct/tests/reconstruction.rs`.
- [x] **Causation is stricter than correlation** —
      `crates/ono-temporal-query/tests/correlation.rs`.
- [x] **Ordering is not causation** — `crates/ono-temporal-query/tests/correlation.rs`.
- [x] **Clock uncertainty is visible** — `crates/ono-temporal-core/tests/ordering.rs`.
- [x] **Provenance survives reconstruction** —
      `crates/ono-temporal-reconstruct/tests/provenance.rs`.
- [x] **Live and historical views share one truth.** Pausing a live map uses the same canonical
      event and state model as a textual temporal query —
      `crates/ono-cli/tests/temporal_view.rs`.
- [x] **System history and shell history are distinct** —
      `crates/ono-cli/tests/temporal_context.rs`.
- [x] **Ono actions are traceable** — `crates/ono-cli/tests/action_causality.rs`.
- [x] **External commands remain honest.** Ono claims process creation and no downstream effect
      an adapter or provider did not report (§17.6) —
      `crates/ono-cli/tests/action_causality.rs`.
- [x] **Retention is bounded** — `crates/ono-temporal-ledger/tests/retention.rs`.
- [x] **Recording is opt-in** — `crates/ono-cli/tests/recorder.rs`.
- [x] **Recording does not escalate privilege** — `crates/ono-recorder/tests/privilege.rs`.
- [x] **Secrets stay out of history** — `crates/ono-temporal-ledger/tests/privacy.rs`.
- [x] **Machine-readable semantics precede rendering.** A renderer consumes canonical query
      ...
      because it can.
- [x] **No fake rewind** — `crates/ono-temporal-render/tests/gap_frame.rs`.

#### 4.11.13 Release criteria (§56) and delivery

- [x] **Every normative temporal command is machine-registered.** `at`, `now`, `present`,
...
- [x] **The shell is unchanged when temporal persistence is disabled.** v0.2–v0.4 behaviour is
...
- [x] **`ono-cli` is not the temporal engine.** The crate layering forbids it and the module
      inventory records the temporal modules — `xtask/src/architecture.rs`,
      `docs/contracts/hardening/module_architecture.yaml`.
- [x] **The temporal documentation of §46 exists.** Evidence-backed time, recorder privacy and
...
- [x] **The fuzz targets of §47.3 exist and run.** Temporal store decoders, event payloads, time
...
- [x] **The acceptance container enables the recorder for the temporal cases.** In a private
      writable home, as the unprivileged user, with no network — `docker/acceptance/cases/`,
      `docker/Dockerfile`.
- [x] **`scripts/release-check.sh` is green.**

### 4.12 The v0.6 tranche — Prospective Change, Protection & Recovery

`docs/specs/ono_sendai_shell_spec_v0.6_prospective_change_protection_recovery.md` puts a semantic
layer between intent and irreversible effect: the `ChangePlan`. A proposed change becomes an
object that can be inspected for what it would touch, what would protect it and what would remain
irreversible, before any of it is made real — and, where evidence supports it, recovered
afterwards. This subsection is its definition of done, written from §55's fifty acceptance
scenarios, §63's twenty release criteria, §2's eighteen core invariants and §56's twenty-two
destructive-recovery safety facts.

**A box is ticked by a named automated proof** — a test that runs un-ignored in `scripts/gate.sh`,
or a case that runs in `scripts/acceptance.sh` — never by judgement, never by reading code.
`xtask/tests/change_evidence.rs` holds this subsection to the tree: every test and case a ticked
box names must exist, and no test it names may be `#[ignore]`d.

Conventions this subsection relies on:

- The case numbers **280–329** belong to this tranche, blocked as 280–284 planning, 285–288 drift
  and rebase, 289–293 protection truth, 294–297 apply and verify, 298–302 recovery, 303–305 resume
  and concurrency, 306–308 bulk and remote, 309–311 spatial and temporal integration, 312–314
  KUANG/11 and security, 315–317 retention and cleanup, 318–320 storage providers against real
  filesystems, 321–323 machine-readable output and scripting.
- A case that does not exist yet is named in plain text without backticks, so
...
- The change, recovery and transaction error families are `Ono-Sendai-E1701`…`E1727`,
  ...
  required it.
- The change verification record is `ono.change-verification/1` and not §46.6's
  `ono.verification-result/1`, which v0.2 §31.36 already holds for `verify plugin`; ADR-0802
  records that one.
- A plan is referenced by an unambiguous prefix of its identity, or as `plan/<id>`; the plan just
  produced is `@`. ADR-0803 records why §5's `@plan` is not added to the lexer, and confirms that
  every spelling §36.4 states normatively works verbatim.
...
- [ ] **This subsection is held to the tree.** Every proof a ticked box names resolves to a test
      ...
      last, because a harvester that resolves nothing looks identical to one that works: its own
      suite mutates the checklist and proves each rule reports what it is for.
- [ ] **The forty-nine refusals of §45 exist in both registries.** Compared bidirectionally on
      code, name and kind by `xtask/src/contracts.rs::check_error_registry`, and indexed by the
      question a reader is asking, with a `nothing_changed` column per code, by
      `xtask/src/change.rs::check_errors` — `xtask/tests/change_contracts.rs`.
- [ ] **Every schema §46 names is registered, embedded and validating.** The ten schemas of §46 and
      the tranche's own, each in `docs/contracts/schemas/`, each embedded in
      `crates/ono-value/src/builtin.rs`, each producing a record that validates against it —
      `crates/ono-change-core/tests/value_records.rs`.
- [ ] **The eleven machine-readable registries of §47 exist and are internally consistent.**
      `docs/contracts/change/{plans,actions,effects,risk,verification,strategies}.yaml` and
      `docs/contracts/recovery/{providers,assets,consistency,policies,errors}.yaml`, each parsing,
      each indexed by the hardening registry inventory, each validated by
      `xtask/src/change.rs::check` — `xtask/tests/change_contracts.rs`.
- [ ] **`spec-check` fails on v0.6 contract drift.** A vocabulary the registry and the machine
      spell differently, a lifecycle edge one of them draws and the other does not, a restore
      method whose destructiveness order differs from the one recovery planning uses, a protection
      level whose persistent-coverage claim differs, an execution method that admits a command
      line, an asset type whose failure-domain claim differs, a stable command missing from either
...
- [ ] **Nothing can raise a confidence, a consistency or a risk class by combining.**
      ...
      `crates/ono-change-core/src/effect.rs`, `crates/ono-change-core/src/protection.rs`,
      `xtask/tests/change_contracts.rs`.
...
- [ ] **Plan creation stays inside §52.1's budget.** A single-host single-service plan seals in
      under 150 ms excluding provider discovery, and a plan over five thousand frozen targets
      completes in bounded memory and round-trips through the store (§52.5) —
      `crates/ono-change-plan/tests/store.rs`.
...
- [ ] **A file changed after seal makes apply refuse.** §55.2 case 6 and §7.3: the refusal is
      ...
      mutated — `crates/ono-change-plan/tests/drift.rs`, case 285-drift-stops-apply.
- [ ] **A service target that disappeared makes apply refuse.** §55.2 case 7:
      `change.target_changed` — `crates/ono-change-plan/tests/drift.rs`,
...
- [ ] **A precondition that could not be observed blocks rather than passing.** §2.4 —
      `crates/ono-change-core/src/target.rs`, `crates/ono-change-plan/tests/drift.rs`.
...
- [ ] **A network filesystem is refused by a local snapshot provider.** Appendix B.6 —
      `crates/ono-change-protection/tests/domain.rs`, case 287-persistence-resolution.
- [ ] **An overlay's writable layer is resolved or the claim is refused.** Appendix B.4 —
...
- [ ] **A recursive snapshot records each dataset individually.** §13.3: one `zfs snapshot -r`
...
- [ ] **The snapshot is created before the first mutation.** §55.3 case 12 and §4.5 —
      `crates/ono-change-executor/tests/prepare.rs`, case 294-prepare-before-mutate.
- [ ] **Snapshot creation failure prevents mutation.** §55.3 case 13 and §2.3: zero mutate actions
      execute and the plan is `PREPARE_FAILED` — `crates/ono-change-executor/tests/prepare.rs`,
...
- [ ] **A full rollback requiring newer snapshot destruction is blocked without acceptance.**
      ...
      `crates/ono-recovery-zfs/tests/rollback.rs`, case 299-destructive-rollback-is-gated.
- [ ] **No argument vector this provider produces carries a recursive destructive flag.** §13.6
      forbids silently adding flags equivalent to removing newer history —
      `crates/ono-recovery-zfs/tests/rollback.rs`.
...
- [ ] **The twelve facts of §56.1 each have a proof, and each is fail-closed.** Exact dataset and
      ...
      requirement, expected discarded live data, sufficient privilege, and explicit acceptance for
      history destruction — with a second test per fact proving that where it cannot be
      established, destructive recovery is blocked with `recovery.plan_incomplete` rather than
      guessed (§56.3) — `crates/ono-recovery-zfs/tests/safety_checklist.rs`.
- [ ] **The pool's free-space floor fails protection closed rather than worsening exhaustion.**
      Appendix D.3 — `crates/ono-recovery-zfs/tests/space.rs`.
- [ ] **A snapshot name is sanitised and stays one argument.** §43.6 and §12.3: a scope containing
      shell metacharacters produces a valid ZFS name and an argument vector of the expected arity —
      `crates/ono-recovery-zfs/tests/naming.rs`.
- [ ] **The provider degrades to unsupported at a version it has not validated.** Appendix G.4 —
      `crates/ono-recovery-zfs/tests/availability.rs`.
- [ ] **The provider parses what real ZFS prints.** The deterministic suite replays recorded
...
- [ ] **A parent snapshot's coverage excludes every nested subvolume beneath it.** §14.3 and
...
- [ ] **A derived writable subvolume is tracked as its own asset.** §14.5 —
      `crates/ono-recovery-btrfs/tests/recovery.rs`.
- [ ] **The recovery plan describes the real method rather than a fake in-place rollback.**
      ...
      `crates/ono-recovery-btrfs/tests/recovery.rs`, case 291-btrfs-recovery-is-a-workflow.
...
- [ ] **A multi-subvolume set does not claim cross-subvolume atomicity.** Appendix D.7: members
      created sequentially compose to the weakest consistency, and the set records that they were —
      `crates/ono-recovery-btrfs/tests/protection.rs`.
- [ ] **The snapshot location is not underneath a subvolume being snapshotted.** Appendix D.8 —
      `crates/ono-recovery-btrfs/tests/protection.rs`.
- [ ] **The ten facts of §56.2 each have a proof, and each is fail-closed.** Filesystem and
      subvolume id, the target snapshot existing and valid, nested subvolume boundaries, whether
      selective restore is possible, whether subvolume replacement is required, default-subvolume
      and boot impact, mount and reboot requirement, later state that would be discarded, correct
      treatment of the read-only snapshot, and no assumption of recursive coverage — each with a
      second test proving §56.3's block — `crates/ono-recovery-btrfs/tests/safety_checklist.rs`.
- [ ] **The provider parses what real Btrfs prints.** The deterministic suite replays recorded
...
- [ ] **The recovery store is private.** §15.3 and §44.3: directory `0700`, files `0600`, applied
      in the creating syscall — `crates/ono-recovery-files/tests/store.rs`.
- [ ] **Restoration is atomic.** §15.4: temp file, fsync, rename — never a truncation in place,
...
- [ ] **Sockets, devices, pseudo-filesystems and oversized trees are refused rather than
      truncated.** §15.2, each with the limit and the measurement named —
      `crates/ono-recovery-files/tests/scope.rs`.
- [ ] **The metadata a restore actually returns is measured rather than asserted.** Appendix C.7:
...
- [ ] **A plan is protected only when every required persistent domain has a validated path.**
      Appendix A.5, and Appendix A.6's worked matrix composes to `PROTECTED` with its exclusions
      shown — `crates/ono-change-core/src/protection.rs`.
- [ ] **An unknown domain caps the plan at partially protected.** Appendix A.7, lifted only by an
      explicit policy declaration recorded in the sealed plan —
      `crates/ono-change-core/src/protection.rs`.
- [ ] **Compensation never satisfies an exact objective.** Appendix A.2: an inverse action cannot
      promise the same bytes back — `crates/ono-change-core/src/protection.rs`.
- [ ] **The smallest sufficient scope wins, not the largest snapshot.** Appendix A.4: a
...
- [ ] **`off` still reports the protection that was available.** §17.2 —
...
- [ ] **A stale recovery asset is not presented as a just-before-change recovery point.** §18.3,
      and an asset with no recorded fingerprint is never fresh —
      `crates/ono-change-protection/tests/freshness.rs`.
- [ ] **No cost path reports a copy-on-write snapshot as free.** §38.2 —
      `crates/ono-change-protection/tests/cost.rs`.
...
- [ ] **A required PREPARE failure means zero mutate actions executed.** §55.7 case 31, §2.3 and
      ...
      refusal says so — `crates/ono-change-executor/tests/prepare.rs`,
      case 294-prepare-before-mutate.
- [ ] **A partial prepare retains the assets it created and mutates nothing.** Appendix F.1: four
      snapshots created and a fifth failing leaves the plan targets untouched, and cleanup failure
      does not obscure the original failure — `crates/ono-change-executor/tests/prepare.rs`.
- [ ] **Mutation succeeding and required verification failing makes the plan FAILED.** §55.7
      case 32 and §2.14 — `crates/ono-change-executor/tests/verify.rs`, case 295-apply-and-verify.
...
- [ ] **A verification timeout is never read as success.** §23.5 —
      `crates/ono-change-core/src/verification.rs`, `crates/ono-change-executor/tests/verify.rs`.
- [ ] **Apply refuses a draft and an expired plan.** §5.6 —
      `crates/ono-change-executor/tests/apply.rs`, case 295-apply-and-verify.
- [ ] **Apply revalidates a sealed plan before mutation.** §5.6 and §7.3 —
      `crates/ono-change-executor/tests/apply.rs`.
...
- [ ] **The failure display does not present recovery as the only next step.** Appendix E.5 —
      `crates/ono-change-render/tests/progress.rs`.
- [ ] **An auto-recovery declaration that does not meet §26.3's six conditions is rejected at
      seal.** §26.3 — `crates/ono-change-recovery/tests/auto.rs`.
...
- [ ] **A recovery whose drift analysis did not run is gated rather than waved through.** §62.8
...
- [ ] **The least-destructive method that satisfies the goal is chosen.** Appendix C.1 and §62.5 —
      `crates/ono-change-core/src/recovery.rs`, `crates/ono-change-recovery/tests/method.rs`.
- [ ] **Automatic semantic merging is not a method this engine can choose.** Appendix C.5 —
      `crates/ono-change-recovery/tests/method.rs`.
- [ ] **A directory restore does not delete newer extra files by default.** Appendix C.6 —
      `crates/ono-recovery-files/tests/restore.rs`.
...
- [ ] **Nothing claims a rollback succeeded.** §25.3: the words `rollback successful`, `fully
...
- [ ] **The recovery plan view shows newer-state impact above the restore detail.** Appendix E.6 —
...
- [ ] **Plan state is reconstructable from persisted action records.** §41.2: a store reopened
      ...
      neither failed nor succeeded (Appendix F.2) — `crates/ono-change-plan/tests/store.rs`,
...
- [ ] **The same plan cannot apply concurrently from two sessions.** §55.9 case 42 and §42.4: the
      ...
      `crates/ono-change-plan/tests/store.rs`, case 304-concurrent-apply.
...
- [ ] **A secret is not persisted in raw form.** §36.3: the store holds an opaque handle, and the
      seal still verifies over it — `crates/ono-change-plan/tests/secrets.rs`.
...
- [ ] **No strategy is unbounded.** §28.4: `parallel` carries its width and a width of zero is
...
- [ ] **The recovery event chain is auditable.** §55.11 case 50 and §22.1 —
      `crates/ono-cli/tests/change_timeline.rs`, case 310-plan-events-on-the-timeline.
- [ ] **v0.6 uses the v0.5 ledger rather than a second history.** No competing audit subsystem
...
- [ ] **Impact is derived from the v0.4 topology with its confidence unchanged.** §9.3 and §3.5:
...
- [ ] **A bounded impact traversal never renders as a complete one.** §9.5 and §52.2 —
      `crates/ono-change-impact/tests/impact.rs`, `crates/ono-change-render/tests/impact.rs`.

#### 4.12.13 KUANG/11, the model broker and security (§43, §44, §48, §49)

- [ ] **The six contribution types of §48.2 exist and are declared on disk.** `ActionProvider`,
      `ImpactProvider`, `RecoveryProvider`, `VerificationProvider`, `RiskRule` and `ChangeView`,
      each readable from the manifest without running the package —
      `crates/ono-kuang-sdk/tests/conformance.rs`, `docs/contracts/kuang/contributions.v1.yaml`.
- [ ] **The capabilities of §48.3 exist in every registry that mirrors them.** `change.plan.read`,
      `change.plan.contribute`, `change.action.execute`, `verification.observe` and the seven
...
- [ ] **A recovery provider that declares `recovery.restore` without a destructive capability is
...
- [ ] **A contributed risk rule can raise the class and never lower it.** §19.2 —
      `crates/ono-kuang-sdk/tests/conformance.rs`.
...
- [ ] **Recovery asset metadata may be shown and contents may not.** §44: the store is private,
      ...
      `crates/ono-recovery-files/tests/store.rs`, case 292-file-recovery.
- [ ] **The v0.6 fuzz targets exist and run.** The plan block grammar, plan and asset
...
- [ ] **The security tests of §54.6 pass.** Symlink swap races, path traversal, privilege
      ...
      `crates/ono-recovery-files/tests/restore.rs`, `crates/ono-change-plan/tests/store.rs`,
      `crates/ono-kuang-sdk/tests/conformance.rs`, case 313-change-security.
- [ ] **Failure is injected at every lifecycle boundary.** §54.5's ten: snapshot creation fails, a
      ...
      fails, storage fills, and a remote host disappears —
      `crates/ono-change-executor/tests/failure_injection.rs`.
...
- [ ] **4. Unknown is preserved.** No combining operation strengthens a confidence, a consistency
...
- [ ] **10. Rollback is not a universal verb.** `recover` is the cross-domain word, and nothing
      claims Btrfs performs a rollback — `crates/ono-recovery-btrfs/tests/recovery.rs`.
- [ ] **11. Cross-provider changes are not atomic by default.** —
      `crates/ono-change-core/src/protection.rs`.
...
- [ ] **16. All lifecycle transitions are auditable.** They reach the v0.5 ledger —
      `crates/ono-cli/tests/change_timeline.rs`, case 310-plan-events-on-the-timeline.
- [ ] **17. No hidden shell scripts.** No execution method admits a command line —
...
- [ ] **All machine-readable contracts match runtime registration.** §63.17 —
...
- [ ] **The shell is unchanged where v0.6 is not used.** v0.2–v0.5 behaviour is green with the
      ...
      existing suites and cases running unmodified.
