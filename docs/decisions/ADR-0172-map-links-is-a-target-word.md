...
`docs/spec/targets.yaml` under the same rule the registry already applies to `link` and `context`:
...
- **Edges**: one `host.linked_to` edge per link — the relation `docs/spec/spatial/relations.yaml`
