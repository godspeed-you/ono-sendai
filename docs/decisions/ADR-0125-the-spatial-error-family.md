...
`docs/spec/errors.yaml` is the single taxonomy: families run E0001–E0002 (parse), E01xx
...
1. **A new family `spatial`, block E10xx**, allocated in the order §40 lists the names:
   ...
   The family is added to the `families` list of `docs/spec/errors.yaml` beside `resolution`,
   `provider` and `adapter`.
2. **The codes live in `docs/spec/errors.yaml`, not in a `spatial-errors.yaml`.** §41's file
...
- The delivering increment adds fourteen entries to `docs/spec/errors.yaml` and fourteen
