...
with constants beside it. `docs/spec/hardening/limits.yaml` held thirteen configuration keys
...
(ADR-0456). `docs/spec/hardening/remote_limits.yaml` — §52.1's second named registry — did not
...
`docs/spec/hardening/limits.yaml` declares for `limits.remote_connections`. `with_max_connections(0)`
...
### 3. `docs/spec/hardening/remote_limits.yaml` exists and holds no numbers
...
> `docs/spec/hardening/remote_limits.yaml` — if §52.1's separate registry is still wanted — should
