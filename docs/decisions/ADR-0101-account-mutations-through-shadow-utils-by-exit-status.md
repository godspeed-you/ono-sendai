...
`docs/spec/commands/identity.yaml` has declared `add|remove|set user` and `add|remove|set group`
...
advertises `user.manage` and `group.manage` (`docs/spec/providers/linux-procfs.yaml`), both
