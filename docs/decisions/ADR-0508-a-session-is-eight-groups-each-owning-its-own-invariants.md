...
- **`session.rs` is one file, not eight.** §31 draws no file layout, unlike §29.2 and §30.2, and
  the file's five capture sites are keyed by path in `docs/spec/hardening/streaming.yaml`. A
  split would move them for no requirement, so it was not made.
- **`docs/spec/hardening/streaming.yaml` moved one entry**: the capture stack is now held by
