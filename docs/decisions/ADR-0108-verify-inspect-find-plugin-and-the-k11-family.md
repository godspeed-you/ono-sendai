...
`KuangRemotePolicyDenied`), rendered exactly as `docs/spec/kuang/errors.v1.yaml` renders them
and listed in `docs/spec/errors.yaml` under their own heading, so `spec-check`'s registry check
