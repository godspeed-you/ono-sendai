...
The user then added `docs/ono_sendai_shell_spec_v0.3_external_command_adapters.md` beside the
...
- the new specification carried no entry in `docs/spec.sha256`, so the immutability guarantee of
...
2. **every** narrative specification has a `sha256sum` line in `docs/spec.sha256` — a file with no
...
- Recording the checksum of a specification the agent did not write is not a weakening of §5.1.
  ...
  guard rather than out from it. AGENTS.md §14 reserves `docs/spec.sha256` for the user in the
