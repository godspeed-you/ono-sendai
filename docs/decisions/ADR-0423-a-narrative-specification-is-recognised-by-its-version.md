...
testing for the prefix `docs/ono_sendai_shell_spec_`. Both matched the shape of the three names
...
- With discovery widened, `spec-check` went red against the tree for the two real defects, and
  they were fixed in the same increment: the v0.5 checksum line is in `docs/spec.sha256` and
