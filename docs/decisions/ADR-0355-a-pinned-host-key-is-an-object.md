...
2. **The verbs are the ones the vocabulary already has**, and their meanings are the trust store's:
   ...
   rule 4 forbids one. `docs/spec/verbs.yaml` needed nothing new, which is the check that this is
   the right shape.
...
Hard: `host-key` is a hyphenated target, which is a first for this vocabulary — `docs/spec/targets.yaml`
...
- **`trust host` / `forget host` as new verbs** — rejected: `docs/spec/verbs.yaml` is a closed
