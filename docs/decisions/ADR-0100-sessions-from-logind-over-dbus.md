...
`docs/spec/commands/identity.yaml` has declared `ono.session.get` as `stable` with
`provider_capability: session.list` and a `--user ref<ono.user/1>` option since Phase D, with
`ono.session/1` parked in `docs/spec/schemas/deferred.yaml`. Nothing answered the target:
...
**`ono.session/1` is written** (`docs/spec/schemas/session.v1.yaml`) and leaves
...
is an `await`; it is declared in `docs/spec/providers/systemd.yaml` as `systemd-logind`, and
...
- `docs/reference/schemas.md` is regenerated; `docs/spec/schemas/deferred.yaml` no longer lists
  `ono.session/1`.
