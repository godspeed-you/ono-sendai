...
- **`Cgroup`** — the control group a process is accounted to (§12, §16.3). It gets a real,
  declared contract, `docs/spec/schemas/cgroup.v1.yaml`, and `compute.cgroups` in
...
Neither is claimed by any provider in `docs/spec/providers/`, which `spec-check` now checks, so
