...
key. `docs/spec/language.yaml` has never written that shape: it writes `argument_modes` as a
...
1. **The registry's shape is the contract; readers conform to it.** `argument_modes` is a
   ...
   (`docs/spec/language.yaml`). `expression_heads()` reads exactly that, and nothing else reads
...
2. **An empty vocabulary is reported, not tolerated.** `expression_heads()` distinguishes "there
   ...
   the second as a problem against `docs/spec/language.yaml`. A check that cannot fail is a check
   that is not running.
...
- The argument-mode check is armed against this repository's own registry, and
  ...
  loads `docs/spec/language.yaml` verbatim so it stays armed. The 171 committed commands pass it.
...
- **Rewrite `docs/spec/language.yaml` into the mapping shape.** Rejected: the file is the public
