...
each classified in its own right in `docs/spec/hardening/streaming.yaml`.
...
`docs/spec/hardening/streaming.yaml` keys every entry on a file and an enclosing item, so a split
