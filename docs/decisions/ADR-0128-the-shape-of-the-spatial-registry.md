...
ADR-0126 fixed *where* the spatial registry lives (`docs/spec/spatial/`). §41.1 and §41.2 fix
...
2. **`spaces.yaml` declares `object_type` as §41.1's example spells it** — for a collection, the
   ...
   - `schema`: the `docs/spec/schemas/` id of the records the place is built from, or **null**
...
5. **`cargo run -p xtask -- spec-check` enforces all of it**: parents resolve, ids are unique,
   ...
   schema exists under `docs/spec/schemas/`, every direction/confidence/cost class is from the
