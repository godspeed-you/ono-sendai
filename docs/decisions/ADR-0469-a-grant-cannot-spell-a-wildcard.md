...
no room for a wildcard.** The grammar is the one `docs/spec/capabilities.yaml` uses: at least two
...
remote-only action taxonomy." The grant names a `docs/spec/capabilities.yaml` id, and the agent
