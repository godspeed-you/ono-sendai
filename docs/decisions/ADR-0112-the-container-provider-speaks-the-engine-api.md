...
no phase for a container provider, so `docs/spec/commands/container.yaml` carried every command
...
`docs/spec/schemas/container.v1.yaml`: identity `[id]` (the engine's full id); `name` is the
...
`docs/spec/schemas/image.v1.yaml`: identity `[id]` (the content digest); `reference` is the
...
`docs/spec/schemas/container-event.v1.yaml` is the ADR-0078 envelope for `watch container`,
keyed `container`.
...
- `docs/spec/providers/container-engine.yaml` declares the provider; the conformance suite
