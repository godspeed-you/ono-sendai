...
Every registry under `docs/spec/` is held against the code that serves it —
`check_error_registry`, `check_commands`, `check_spatial_registry`,
`check_provider_claims` — except the seven `docs/spec/kuang/*.v1.yaml`, which reached
...
- **Generating the contract from the code** — inverts the authority order: `docs/spec/` is the
