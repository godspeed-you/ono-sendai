...
`spec/adapters/` (read `docs/spec/adapters/`, AGENTS.md §2) so that reference pages,
...
1. **One file per package**, `docs/spec/adapters/first-party/<package>.yaml`, format
   `ono-adapter-pack/1`, described field-for-field by `docs/spec/adapters/schema.yaml` in the
   style of `docs/spec/kuang/manifest.v1.yaml`. A package carries `package` (id, name,
...
6. **Schemas are canonical.** `schema` must name a registered `ono.*/1` schema; an adapter that
   needs a new one adds it to `docs/spec/schemas/` in the same increment, and the ADR-0027 list
...
7. **Fixtures are part of the contract.** `fixtures` names a directory under
   `docs/spec/adapters/fixtures/<package>/<adapter>/`; each fixture is `<name>.out` (the bytes
