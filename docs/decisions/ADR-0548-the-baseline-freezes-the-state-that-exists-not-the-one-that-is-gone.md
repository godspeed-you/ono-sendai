...
- **H7** wrote `docs/spec/hardening/performance_baseline.json` — §32.4's regression baseline, six
...
`docs/spec/hardening/`, so `registries.yaml` does not index it (ADR-0547). That is right —
`docs/spec/` is the public contract surface and a snapshot of one tranche is a record, not a
