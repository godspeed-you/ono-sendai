...
- Spec refs: v0.4.1 §2.3 (safety mechanisms fail closed), §15.1 (the three concepts), §16.1
  ...
  AGENTS.md §2 (`docs/contracts/`), §6 (registries are the public contract); ADR-0283, ADR-0430
...
**The table is `docs/contracts/hardening/kuang_confinement_controls.yaml`, and the gate compares it
against `ono_kuang_protocol::confinement` in both directions on every run.**

**1. It is a registry under `docs/contracts/hardening/`, not under `docs/contracts/kuang/`.** The seven
`docs/contracts/kuang/` contracts are the *plugin API*: what a package author writes and what the two
...
`docs/contracts/hardening/kuang_confinement_controls.yaml`, and #117 collects the other six into the
...
**The table in `docs/contracts/kuang/lifecycle.v1.yaml`, beside `isolation_tiers`.** Tempting, because
