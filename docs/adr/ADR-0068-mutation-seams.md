...
1. **`set` and `remove` were claimed whole by the shell.** `crates/ono-cli/src/resolve.rs`
   ...
   `ono.service.set`, `ono.dir.set` and more (`docs/contracts/commands/`); none of them could ever be
   reached, whatever a provider implemented.
2. **The ActionResult row did not match its own schema.** `docs/contracts/schemas/action-result.v1.yaml`
   ...
   `io.not_found` (`docs/contracts/schemas/error.v1.yaml`). `stop process 1 | to json` wrote
...
verb is `mutating` in `docs/contracts/verbs.yaml`, it names a `provider_capability`, its phase is
delivered, and a provider registered for its target advertises that capability
(`Provider::capabilities()`, vocabulary in `docs/contracts/capabilities.yaml`). `builtin_commands`
