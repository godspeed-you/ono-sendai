...
- Status: accepted
- Date: 2026-08-29
- Spec refs: §31.64, §31.22, §31.68, `docs/contracts/kuang/contributions.v1.yaml`
...
`docs/contracts/kuang/contributions.v1.yaml` adds two rules the narrative leaves implicit: origin is
...
`docs/contracts/commands/*.yaml` and had no room for anything else, so a contributed command could not
...
- `RawCommand::into_contract` always produces `Origin::Core`. A contract document never names its
  own origin, whether it was read from `docs/contracts/commands/` or from a package's
