...
Spec §31.78 asks for a machine-readable source tree under `spec/kuang/` — `docs/contracts/kuang/` in
...
Nothing in `docs/contracts/kuang/` describes an implementation. It describes what Phase I builds.

## Decision

### 1. Eight contract files plus a schema directory, not §31.78's seventeen

`docs/contracts/kuang/` holds `manifest.v1.yaml`, `capabilities.v1.yaml`, `protocol.v1.yaml`,
...
| `capabilities.yaml` | `capabilities.v1.yaml` (scope shapes and lifecycle; the family list stays in `docs/contracts/capabilities.yaml`) |
...
### 2. The family list stays in `docs/contracts/capabilities.yaml`; this directory adds the shapes
...
grant lifecycle — and declares `docs/contracts/capabilities.yaml` authoritative where the two could
...
`docs/contracts/capabilities.yaml` gives `history.write` no scope keys. The two are reconciled rather
...
This contradicts a registry that already exists: `docs/contracts/commands/kuang.yaml` documents
...
`docs/contracts/schemas/deferred.yaml` defers eleven KUANG/11 schemas. Writing them turned up three
...
- Why: spec §43 spells every other code in the product `Ono-Sendai-E0001`, and
  `docs/contracts/errors.yaml` implements that spelling. §31.79 says these codes "should integrate
...
`docs/contracts/commands/kuang.yaml` already promise to emit. Spec §31.77's ambition — that a
...
`docs/contracts/capabilities.yaml`, every schema reference against the schema directory, every error
code against `errors.v1.yaml`, every command id against `docs/contracts/commands/`) — plus the rule
...
1. Eleven of the fourteen schemas belong under `docs/contracts/schemas/`, and their
   `docs/contracts/schemas/deferred.yaml` entries must be removed when they move. Three
   (`ono.evidence/1`, `ono.recommendation/1`, `ono.assistant-action/1`) are new and have no
   deferred entry to remove.
2. `docs/contracts/commands/kuang.yaml`'s `get plugin --state` documentation says `running`; it must
   say `active`, and should list `degraded` and `quarantined` (§5 above).
3. `docs/contracts/commands/kuang.yaml` has no `ono.plugin.set`, although spec §31.3 writes
...
4. Spec §31.3's requirement — that if `install` and `verify` are accepted as core verbs they
   ...
   grammar" — is **already satisfied**: `docs/contracts/verbs.yaml` carries `install`, `load`,
   ...
   re-open it.
5. The KUANG/11 error family belongs in `docs/contracts/errors.yaml` alongside the E-codes, or that
   file must reference `docs/contracts/kuang/errors.v1.yaml` explicitly. One error registry that a
   reader has to know is in two files is a registry with a seam.
6. `examples/` (§16 above) arrives with the test host.
7. `docs/contracts/providers/*.yaml` conformance generation (ADR-0012 §1) will need a KUANG/11
   equivalent for contributed providers, driven by §31.74.

Encoded by: `docs/contracts/kuang/**`, and the `spec-check` cross-reference checks Phase D adds and
Phase I extends.
...
- **Writing §31.78's seventeen files verbatim** — rejected: several would hold three lines, the
  ...
  by its §31.78 name.
- **Repeating the capability families in `docs/contracts/kuang/capabilities.v1.yaml`** — rejected:
