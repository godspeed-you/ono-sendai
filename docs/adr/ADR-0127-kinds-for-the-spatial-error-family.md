...
`docs/contracts/errors.yaml` may only use a kind the registry itself declares.
...
- `crates/ono-core/tests/error_taxonomy.rs::should_carry_the_spatial_family_of_the_v04_specification_when_enumerated`
  pins the fourteen (code, name, kind) triples, and `spec-check` holds `docs/contracts/errors.yaml`
