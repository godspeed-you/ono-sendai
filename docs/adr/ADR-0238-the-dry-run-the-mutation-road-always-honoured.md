...
And no command in `docs/contracts/commands/` declared the option, so:
...
- **Contribute `--dry-run` implicitly for every mutating verb, in the binder rather than in the
  ...
  `docs/contracts/` is the contract, and `docs/reference/`, help and completion are generated from it.
