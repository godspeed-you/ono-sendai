...
- `docs/specs/spec.sha256` records the checksum of the specification. `cargo xtask spec-check`
...
- Restoring the file is the only correct response to a red immutability check. Updating
  `docs/specs/spec.sha256` is the user's action, taken when they deliberately replace the spec.
