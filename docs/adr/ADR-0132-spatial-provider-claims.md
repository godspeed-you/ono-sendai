...
`docs/contracts/providers/*.yaml`, beside its targets, capabilities and schemas. That registry is
...
three-value vocabulary declared in `docs/contracts/spatial/spatial.yaml` under `provider_claims`
...
- **A separate `docs/contracts/spatial/providers.yaml`.** Rejected: two documents that describe the
