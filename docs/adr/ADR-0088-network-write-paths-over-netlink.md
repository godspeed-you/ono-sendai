...
`docs/contracts/commands/network.yaml` declared nine write paths — `add`/`remove`/`set route`,
...
`docs/contracts/commands/*.yaml` gains an optional command-level field `confirmation`, `bulk` by
