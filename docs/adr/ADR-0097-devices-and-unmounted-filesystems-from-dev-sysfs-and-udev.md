...
`docs/contracts/commands/storage.yaml` declares `get device` (output `stream<ono.device/1>`,
...
`docs/contracts/schemas/device.v1.yaml`: identity `[path]`; fields `path`, `name`, `kind`
...
- The provider registry gains `linux.sysfs` (`docs/contracts/providers/linux-procfs.yaml`);
