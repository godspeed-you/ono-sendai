...
`ono.limits.inspect` is declared in `docs/contracts/commands/meta.yaml` against a new
...
in both directions against `docs/contracts/hardening/limits.yaml` — a key the shell enforces and the
