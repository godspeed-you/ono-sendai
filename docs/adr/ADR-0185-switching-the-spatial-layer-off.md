...
- The refusal is raised where the shell binds a native stage
  ...
  `ono.place.` — the thirteen commands of `docs/contracts/commands/spatial.yaml`. It is raised for a
...
- A future spatial command must carry an `ono.place.*` contract id to be covered. That is
  already the naming rule of `docs/contracts/commands/spatial.yaml`, and `spec-check` holds the file
