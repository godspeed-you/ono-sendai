...
**The three crates that embed `docs/contracts/` documents — `ono-command`, `ono-value`,
`ono-adapter` — transcode them from YAML to JSON in a `build.rs`, embed the JSON, and read that at
run time.**

1. **The YAML stays the source.** `docs/contracts/commands/*.yaml`, `docs/contracts/schemas/*.yaml`,
   `docs/contracts/verbs.yaml`, `targets.yaml`, `capabilities.yaml` and
   `docs/contracts/adapters/first-party/*.yaml` are unchanged in role: people read and edit them,
