...
- `ono.spatial-place/1` is registered in `docs/contracts/schemas/`, embedded in `ono-value`, and named
  by `docs/contracts/targets.yaml`'s `place` target and `docs/contracts/commands/spatial.yaml`.
