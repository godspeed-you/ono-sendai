...
`docs/contracts/commands/network.yaml` declares `ono.dns.resolve` (`resolve dns <query>`, output
`stream<ono.dns-record/1>`) and `ono.port.test` (`test port <host> <port>`, output
`ono.probe-result/1`) as stable Phase C commands, and `docs/contracts/schemas/deferred.yaml` carried
...
`ono.dns-record/1` (`docs/contracts/schemas/dns-record.v1.yaml`) has the three fields the resolver
...
- `docs/contracts/providers/linux-resolver.yaml` declares the two providers (`linux.resolver`,
