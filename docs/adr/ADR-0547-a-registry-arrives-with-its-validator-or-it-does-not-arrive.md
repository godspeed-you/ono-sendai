...
this issue was worked, `docs/contracts/hardening/` held **fifteen** files, because the rest arrived with
...
**`docs/contracts/hardening/registries.yaml` indexes the directory, and a contract with no row in it
fails the gate.**
...
| --- | --- |
| `limit_key` | the keys `docs/contracts/hardening/limits.yaml` declares |
| `field` | the accessor `ono_protocol::Limits` answers with |
| `refusal` | the stable error names `docs/contracts/errors.yaml` declares (`null` where the peer is dropped before a frame can carry one) |
