...
Registered commands may declare their mode in `docs/contracts/commands.yaml` (phase D); the table
...
The authoritative form is `docs/contracts/grammar.ebnf`, committed with this ADR and checked by
...
Expression-mode table, which is a parser change until `docs/contracts/commands.yaml` drives it in
phase D.
...
diagnostic snapshots; `docs/contracts/grammar.ebnf`.
