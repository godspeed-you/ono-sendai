...
**`docs/contracts/hardening/refusals.yaml` is the census.** One row per hardening refusal, naming the
...
- `boundary` is a §6.1 id validated for shape rather than membership, because §6.1's inventory —
  `docs/contracts/hardening/security_boundaries.yaml` — is owed by issue #118 and does not exist. When
...
- **Add `boundary:` to every row of `docs/contracts/errors.yaml`.** Rejected: the errors registry is
  ...
  under `docs/contracts/hardening/`.
