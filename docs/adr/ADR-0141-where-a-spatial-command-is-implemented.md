...
the built-in implementations**, against the ids `docs/contracts/commands/spatial.yaml` declares.
...
The contract stays in `docs/contracts/commands/`, so `help`, completion, `explain` and `spec-check` see
