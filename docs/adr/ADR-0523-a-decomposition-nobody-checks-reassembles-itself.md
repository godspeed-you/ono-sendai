...
`docs/contracts/hardening/module_architecture.yaml` joins the six registries §52.1 names and the ones
...
- Adding a module to the parser, the evaluator, native execution or `ono-cli` now requires editing
  ...
  `docs/contracts/errors.yaml` imposes on a new error code.
