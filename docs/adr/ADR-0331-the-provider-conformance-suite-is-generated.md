...
`crates/ono-cli/tests/providers.rs` compared `docs/contracts/providers/*.yaml` with the built registry
...
`docs/contracts/providers/*.yaml`, `docs/contracts/schemas/*.v1.yaml`, `docs/contracts/capabilities.yaml` and
`docs/contracts/commands/*.yaml`.** The file is committed; `spec-check` regenerates it and fails when
...
4. **Account** — every declared capability is reached by something: a read capability by the
   snapshot case of the target it reads, any other by a command in `docs/contracts/commands/` that
