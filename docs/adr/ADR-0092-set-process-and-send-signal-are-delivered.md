...
`docs/contracts/commands/process.yaml` carried two `planned` entries with `validation_required:
...
- `docs/contracts/providers/linux-procfs.yaml` declares `process.set` and the `signal` target.
