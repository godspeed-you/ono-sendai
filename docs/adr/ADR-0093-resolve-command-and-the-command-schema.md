...
`docs/contracts/commands/meta.yaml` declares `ono.command.resolve` — "report what a head word
...
2. **`describe` walks the evaluator's own order.** Keywords are the statement keywords of
   `docs/contracts/language.yaml`; functions and aliases are the session's definitions; natives are
...
3. **`ono.command/1` is written down** in `docs/contracts/schemas/command.v1.yaml` and removed from
