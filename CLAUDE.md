...
- **Project:** KUANG, an object-pipeline shell in Rust. Narrative spec:
  `docs/kuang_shell_spec_v0.1.md` (normative MUST/SHOULD/MAY). Machine-readable contracts live
  in `docs/spec/`.
...
- **Method:** strict TDD. `RED → GREEN → REFACTOR → GATE → RECORD → LOOP` (AGENTS.md §6).
  No production code without a failing test first.
- **Pragmatism:** solve the problem in front of you. Fixes, features, refactors and
  optimisations are separate changes and separate commits — never mixed (AGENTS.md §3).
...
- **Autonomy:** every decision not fixed by the spec is yours. Decide, write an ADR in
  `docs/decisions/`, continue. Do not ask the user; do not idle (AGENTS.md §7).
- **Referee:** the test suite plus the quality gate (AGENTS.md §9) decide whether a (sub)goal is
  reached and the next step may start.
- **State board:** `docs/STATE.md` — read first, update last, every session (AGENTS.md §8).
...
to request permission for decisions that AGENTS.md §7 already delegates to you.

**AskUserQuestion.** Effectively banned for this project. Architecture, library and design
questions are answered by you with an ADR. Use it only for the escalation cases in AGENTS.md §7.
...
- `general-purpose` agents for one crate each, following AGENTS.md §12 claiming rules.
...
**Long spec file.** `docs/kuang_shell_spec_v0.1.md` is ~3300 lines. Do not read it whole; use
